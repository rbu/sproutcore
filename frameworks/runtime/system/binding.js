  ...
  */
  flushPendingChanges: function() {
    ...
    while ((queue = this._changeQueue).length > 0) {
      if (log) SC.KVOLogging.triggerChangedBindings();
      ...
      // have triggered.
      if (log) SC.KVOLogging.endTriggerChangedBindings();
    }
  ...
  */
  applyBindingValue: function() {
    ...
    if (!this._oneWay && this._fromTarget) {
      if (log) SC.KVOLogging.setForwardBindingValueToTransformedValue(this, v, tv);
    ...
    if (this._toTarget) {
      if (log) SC.KVOLogging.setBackwardBindingValueToTransformedValue(this, v, tv);
      if (bench) SC.Benchmark.start(this.toString() + "<-") ;
